...
impl InputSettingsGui {
    ...
    fn key_map_ui(map_request: &mut Option<MapRequest>, ui: &mut Ui, available_configurations: &HashMap<InputId, InputConfigurationRef>, joypad_input: &JoypadInput, selected_configuration: &mut InputConfigurationRef, pad: usize) {
        ...
        Grid::new(format!("joypadmap_grid_{}", pad))
                    ...
                    .show(ui, |ui| {
                        use JoypadButton::*;
                        InputSettingsGui::button_map_ui(map_request, ui, input_configuration, joypad_input, Up);
                        InputSettingsGui::button_map_ui(map_request, ui, input_configuration, joypad_input, Down);
                        InputSettingsGui::button_map_ui(map_request, ui, input_configuration, joypad_input, Left);
                        InputSettingsGui::button_map_ui(map_request, ui, input_configuration, joypad_input, Right);
                        InputSettingsGui::button_map_ui(map_request, ui, input_configuration, joypad_input, Start);
                        InputSettingsGui::button_map_ui(map_request, ui, input_configuration, joypad_input, Select);
                        InputSettingsGui::button_map_ui(map_request, ui, input_configuration, joypad_input, B);
                        InputSettingsGui::button_map_ui(map_request, ui, input_configuration, joypad_input, A);
                    });
    }
        ...
        match map_request {
            ...
            _ => {
                ...
                if ui.button(key_to_map).clicked() {
                    *map_request = Some(MapRequest { input_configuration: input_configuration.clone(), button });
                }
            }
        }
...
impl GuiComponent for InputSettingsGui {
    ...
    fn ui(&mut self, ctx: &Context, game_runner: &mut GameRunner) {
        ...
        .show(ctx, |ui| {
            ui.horizontal(|ui| {
                ui.vertical(|ui| {
                    InputSettingsGui::key_map_ui(&mut self.mapping_request, ui, &game_runner.settings.input.configurations, &game_runner.inputs.p1, &mut game_runner.settings.input.selected[0], 1);
                });
                ui.vertical(|ui| {
                    InputSettingsGui::key_map_ui(&mut self.mapping_request, ui, &game_runner.settings.input.configurations, &game_runner.inputs.p2, &mut game_runner.settings.input.selected[1], 2);
                });
            });
        });
