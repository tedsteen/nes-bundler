...
impl Node {
    ...
    pub(crate) async fn get_providers(self: &Self, key: &str) -> HashSet<PeerId> {
        loop {
            match self.try_get_providers(Key::new(&key.to_string())).await {
                ...
                Err(err) => {
                    eprintln!(
                        "Failed to get providers {:?} - ({:?}), retrying...",
                        key, err
                    );
                    tokio::time::sleep(std::time::Duration::from_millis(200)).await;
                }
    ...
    async fn setup_discovery() -> (CommandBus, PeerId) {
        ...
        fn catch_event<T, F>(
            query_id: Result<QueryId, libp2p::kad::store::Error>,
            mut event_bus: tokio::sync::broadcast::Receiver<KademliaEvent2>,
            responder: Responder<T>,
            func: F,
        ) where
            F: Fn(KademliaResponse) -> Option<Result<T, String>> + Send,
            T: Send + 'static,
            F: 'static,
        {
                    ...
                    Poll::Ready(Some(command)) => {
                        match command {
                            ...
                            Command::GetProviders(key, responder) => {
                                ...
                                let local_provides =
                                    swarm.behaviour().local_provides.contains(&key);
